...
if [[ -d "clusters/$ENVIRONMENT/$CLUSTER" ]]; then
    ...
    ./kustomize build --load_restrictor LoadRestrictionsNone apps/admin/traefik-crds > CustomResourceDefinition-treafik.yaml
    ./kustomize build --load_restrictor LoadRestrictionsNone apps/monitoring/kube-prometheus-stack-crds > CustomResourceDefinition-kube-prometheus-stack.yaml
